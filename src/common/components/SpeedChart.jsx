...
const SpeedChart = ({ deviceId }) => {
  ...
  if (!state.data.length) {
    return <Typography color="textSecondary">{t('statusChartNoData')}</Typography>;
  }

  return (
    <ResponsiveContainer width="100%" height={210}>
      <LineChart data={state.data} margin={{ top: 12, right: 16, bottom: 4, left: 0 }}>
        <XAxis
          dataKey="time"
          type="number"
        ...
        />
        <Line type="monotone" dataKey="speed" stroke="#039BE5" strokeWidth={2} dot={false} />
      </LineChart>
    </ResponsiveContainer>
  );
};
