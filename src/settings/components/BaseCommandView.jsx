...
const BaseCommandView = ({
  ...
  useAsyncTask(
    async ({ signal }) => {
      if (includeSaved) {
        const savedResponse = await fetchOrThrow(
          `/api/commands/send?deviceId=${deviceId}`,
          {
            signal,
          },
        );
        const saved = await savedResponse.json();
        let combined = saved.map((it) => ({
          ...it,
          optionType: "saved",
          key: `saved-${it.id}`,
        }));
    ...
    <>
      <Autocomplete
        ...
        value={
          savedId
            ? options.find(
                (it) => it.optionType === "saved" && it.id === savedId,
              ) || null
            : options.find(
                (it) => it.optionType === "type" && it.type === item.type,
              ) || null
        }
        onChange={handleSelect}
        renderInput={(params) => (
          <TextField {...params} label={t("sharedType")} />
        )}
      />
